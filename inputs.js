...
function renderMilitary(root) {
  ...
  const ssABreakeven = () => {
    ...
    const monthly67 = m.ssAFull67;                  // field stores monthly
  ...
  const ssAOut = readonlyField(`Annual benefit at age ${m.ssAClaimAge}:`,
    formatDollars(m.ssAFull67 * 12 * SS_FACTORS[m.ssAClaimAge]));
  const recalcSSA = () => {
  ...
  const ssAFull67Field = field(`${nameA} — Monthly Benefit at FRA (67)`,
    boundInput("dollar", () => m.ssAFull67, (v) => (m.ssAFull67 = v), { onCommit: recalcSSA }),
    { note: "Enter the monthly amount shown on ssa.gov/myaccount — the app converts to annual." });
  ...
  const ssBOut = readonlyField(`Annual benefit at age ${m.ssBClaimAge}:`,
    formatDollars(m.ssBFull67 * 12 * SS_FACTORS[m.ssBClaimAge]));
  const recalcSSB = () => {
  ...
  const ssBFull67Field = field(`${nameB} — Monthly Benefit at FRA (67)`,
